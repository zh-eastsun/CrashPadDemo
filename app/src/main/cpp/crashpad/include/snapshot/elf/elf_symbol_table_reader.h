...
class ElfSymbolTableReader {
 public:
  ...
  struct SymbolInformation {
    ...
    //! \brief Specifies the type of symbol. Possible values crashpad.include
    //!     `STT_OBJECT`, `STT_FUNC`, etc.
    uint8_t type;

    //! \brief Specifies the default scope at which a symbol takes precedence.
    //!     Possible values crashpad.include `STB_LOCAL`, `STB_GLOBAL`, `STB_WEAK`, or
    //!     OS/processor specific values.
    uint8_t binding;

    //! \brief Together with binding, can limit the visibility of a symbol to
    //!     the module that defines it. Possible values crashpad.include `STV_DEFAULT`,
